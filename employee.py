...
class employeeClass:
    def __init__(self, root):
        ...
        self.txt_address.place(x=150,y=300,width=300,height=78)
        ...
        emp_frame.place(x=0,y=410,relwidth=1,height=139)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                messagebox.showerror("Error","Employee ID must be requred",parent=self.root)
            else:
                cur.execute("select * from employee where eid=?",(self.var_emp_id.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Emploee ID is already assigned , try diffrent",parent=self.root)
                else:
